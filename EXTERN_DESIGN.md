...
> DSK con máquina `P341`). **mld128** soportado y **runtime VERIFICADO en emulador
> (5 jul 2026)**: MLD ya arranca y una rutina nativa que escribe un array bancado vía
> `CYD_POKE` funciona en mld128 (ver `doc/dev/MLD_WRITABLE_ARRAYS.md`). **mld estricto**
> no soportado (allocator de 1 banco). Detalle §8.1–§8.3.
...
- **mld128**: **soportado y runtime VERIFICADO en emulador (5 jul 2026)**. Reutiliza
  el mismo camino `$7FFD` que 128k: la rutina, colocada en un banco paginado, queda en
  `$C000` y se alcanza con `SET_RAM_BANK`. El bytecode vive en slots Dandanator
  (`$0000-$3FFF`), **independiente de `$C000`**, así que paginar `$C000` para la rutina
  no lo perturba (confirmado en emulador). Se define `OP_EXTERN_BANKED` también en
  `get_asm_mld128`. El target mld **sí está expuesto en el CLI** (`{48k,128k,plus3,mld,
  mld128}`). Verificación: una rutina nativa escribe un array bancado vía `CYD_POKE` y
  el opcode lo lee de vuelta (`scratchpad/ramtest/mld_broker.py`; ver
  `doc/dev/MLD_WRITABLE_ARRAYS.md`).
...
**Pendiente:** soporte de rutinas nativas en mld estricto (si se decide; hoy error
limpio). (mld128 en runtime y la fase 4 ya HECHOS.)
